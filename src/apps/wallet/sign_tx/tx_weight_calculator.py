...
#
# Heavily inspired by https://github.com/trezor/trezor-mcu/commit/e1fa7af1da79e86ccaae5f3cd2a6c4644f546f8a
#
...
class TxWeightCalculator:
    ...
    def add_witness_header(self):
        if not self.segwit:
            ...
            self.counter += self.ser_length_size(self.inputs_count)  # todo correct?
            self.segwit = True

    def add_input(self, i: TxInputType):

        if i.multisig:
            multisig_script_size = _TXSIZE_MULTISIGSCRIPT + i.multisig.pubkeys_count * (1 + _TXSIZE_PUBKEY)
            input_script_size = 1 + i.multisig.m * (1 + _TXSIZE_SIGNATURE) + self.op_push_size(
        ...
        if i.script_type == InputScriptType.SPENDADDRESS or i.script_type == InputScriptType.SPENDMULTISIG:
            input_script_size += self.ser_length_size(input_script_size)
            self.counter += 4 * input_script_size

        elif i.script_type == InputScriptType.SPENDWITNESS or i.script_type == InputScriptType.SPENDP2SHWITNESS:
